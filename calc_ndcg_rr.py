...
def group_exec(grps,eval_topics, grouped, qrels, filter=False):
    ...
    # for modelname in ['tctcolbert', 'bm25_tctcolbert']:
        ...
        for threshold in [0, 30, 60, 90]:
            ...
            rscore_csv = f'{nfs_dir}/{modelname}/df_{modelname}_{origin_topics}_rscore_{threshold}.csv'
            ...
            for group_id, queries_df in grouped:
                ...
                scoredF = f'{root_dir}/{modelname}/groups/{modelname}_{eval_topics}_{grps}_G{group_id}_T{threshold}.csv'
            ...
            df_threshold = pd.DataFrame(res, columns=['modelname', 'threshold','grps', 'group_id', 'mean', 'std', 'gini',
                                                      'nDCG@10', 'RR'])
            res_csv = f'{root_dir}/{modelname}/groups/new_clustered/result_{modelname}_{eval_topics}_{grps}_{group_id}_T{threshold}.csv'
        ...
        res_csv = f'{root_dir}/allresults/result_{modelname}_{eval_topics}_{grps}_stats.csv'
        print(f'saving {res_csv}')
...
def cal_groups():
    ...
    for nc in num_clusters:
        ...
        # group_exec(nc,'dev_all', dev_all, dev_qrels, filter=False)
        ...
        group_exec(nc, 'dev_topics_grps', dev_topics_grps, dev_qrels, filter=True)
